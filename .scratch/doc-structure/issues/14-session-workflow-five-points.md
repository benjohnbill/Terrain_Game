...
|---|---|---|
| ~~**Ticket 08's migration**~~ — **DISCHARGED 2026-08-03, same day, by the peer session** (`77d892f`). The trigger fired within hours: that lane closed its fog batch, migrated the file, emptied `TICKET_GRANDFATHERED`, and moved the test pin from 1 to 0 with the reason in its assertion message. Worth recording because the discharge was performed by a session that did not build the mechanism, on the mechanism's first day | — | This row goes at the next edit of this table; it is kept once as evidence that the trigger notation works |
| **Part 2 gains a `Status` column** (R5) — `l3-playable-build/DECISIONS-OWED.md` | **Trigger has fired** (the lane is idle as of `77d892f`), so this is takeable now. Note that ADR 0048 closed four Part 2 conflicts, which is exactly the state the column has to be able to express | The column exists and decision blockers are readable from it |
