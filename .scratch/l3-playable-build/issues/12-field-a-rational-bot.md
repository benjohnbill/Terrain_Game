...
- [ ] **The disposition governs three axes** (user ruling 2026-07-25): (a) what share of the commit stack goes to reconnaissance; (b) how it judges inside the confidence band — a **ratio** of optimistic to pessimistic to middling reads, not a single fixed point; (c) how often it takes a given action under similar conditions. No hardcoded constant stands in for any of the three. This **widens** the sealed disposition (tactical-plan-ai ruling ②, which sets a single λ point), so the seal is amended rather than merely applied — see `DECISIONS-OWED.md` R4.
- [ ] **Play varies match to match, and every draw comes from the injected seed.** One disposition ships, but identical play across matches is a failure: the action frequencies plus seeded randomness supply the variation. `Math.random()` and `Date.now()` remain barred (ADR 0040, gate 02 C02.10 — the reason the archive bot was discarded); a different seed yields different play while the same seed replays identically, which is what the seed exists for.
