...
*Added 2026-08-05 by the coverage grill — fog `MAGNITUDE.md` FG-M② and `RULINGS.md` ⑤, plus ADR 0054:*

- [ ] **The coverage projection is re-cut to FG-M② before the band is rendered**, not after. `CoverageView` and `projection/project.ts` still ship ticket 08's counting rule — a sector scores whole if it holds **any** testimony — which reports high while its statements have aged into uselessness. FG-M② replaces it with the mean share of each sector's prior ignorance removed, read off band width, so decay is free off 노화 헌법 P3. Three assertions under `game/tests/` pin the old rule and move with it. Registered in `docs/SYNC-DEBT.md`; that row is deleted by this item, and 08 could not do it because 08 is `resolved` and no ticket authorised the change.
- [ ] **The coverage band carries no number and its imprecision is in the visual** (ruling ⑤ decisions 1–2): no percentage, no `28 / 49`, one band that **fades**, and the fading is what says this is not a measured value. A device that re-sharpens what the quantity blurred — tick marks, snapping, a numeric tooltip — contradicts the ruling. Colour, geometry and position in the strip are this ticket's to choose.
- [ ] **The coverage band does not read as "how much I know"** (ruling ⑤ decision 3). It is the **sector side only** and structurally must be — ④ decision 6 refuses force contacts a total, so no fraction over them exists. Tight readings on every enemy sector while the field army is lost therefore shows a **bright** band, correctly. Make the scope legible either by a name beside it or by seating it against the contacts read so the contrast does the work; which one is this ticket's call, but one of them is owed.
- [ ] **The ground's economy is read on the map, not in the strip** (ADR 0054): per sector, what it **earns** (Public, exact), what it **can raise** (Public, exact), and what it **has already raised** (banded, decays) — as a **summoned lens**, never painted, because fog ② seals a calm map. No treasury figure, no treasury band, no "unknown" row anywhere. The exact layers never blur and only *has raised* ages; that mix is the read, not a defect to reconcile. Today `수입` renders in an off-map per-realm table (`App.tsx`), which is the gap this closes.
