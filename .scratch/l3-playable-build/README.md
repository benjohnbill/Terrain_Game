...
Current state: **the loop is closed** — tickets 01, 02 and 03 landed 2026-07-25 under
the R6 waiver; **05 and 06a landed 2026-07-26**; **06b and 06c landed 2026-07-28**;
**06e created and landed 2026-07-31** (the geography-battle grill's implementation
home, and 06d's blocker); **06d landed 2026-07-31**, so ground changes hands; and
**07 resolved 2026-08-01 — a capital falls and the match ends.** A complete match can
now be played from setup to victory, so every remaining ticket *thickens* a
terminating game rather than building toward one (06b's supply half deferred to R16 by
the 2026-07-28 re-cut). Every other ticket retains its recorded status behind the
values in `DECISIONS-OWED.md`
...
| 06e | ✅ | ✅ **created 2026-07-31 by the geography-battle grill; zero unlanded values.** ADR 0046 (siting on presence, the hex-arc approach contract, the per-sector commit key), terrain-cradle TC-⑮ (the `terrainLayer` → M5 binding, amending TC-⑬'s terrain column), war-model-build WM-⑤ (rout displacement). Every terrain value is cited from M5; nothing is originated. Frontage, interception and Encirclement are explicitly forbidden ground | **LANDED 2026-07-31** — `b591f4e`. Battle-capable sectors 27/56 → 56/56; the commit key moved to the sector across Runtime, preview, projection and UI together; WM-⑤ gave `escaped` its consumer. Two readings the authority left open are recorded in the ticket's § Reading the seams rather than left to be rediscovered |
| 06d | ✅ | ✅ — recomputed and held at claim time. **A second grain ruling was owed and taken 2026-07-31**: moving the register to sector grain was not implementable alone, because `OriginComposition` shares its key and `availableCivilians = register − serving` joins them, so a half-move threw on *every* casualty. Origin moved with it (ADR 0047, landed in parallel). R18's garrison-regen rate stayed moot, R19 stayed upstream | **LANDED 2026-07-31** — merge `34d728d`. Control moves on a won battle, `homeland` ends limbo at ADR 0022's first stable turn, ripening runs 50/60 +10pp to the authored value, and the register's three laws are tested apart. **Two items reported open rather than absorbed**: the economy re-measurement (trigger = ticket 13, thresholds = gate 10) and **garrison → field posture transfer, HELD pending a user ruling** on the wear ledger across a posture change — the naive reading launders wear for free and every fix needs a statement no seal supplies. Both on `docs/SYNC-DEBT.md`; the held intent is unwired and a test pins the rejection. The two-axis review earned its keep twice: origin composition was drifting on every transfer (two apportionments conserve the total, not the composition), and it caught the laundering |
| 07 | ✅ | ✅ **all three cleared 2026-07-31.** Battle-site reachability: ADR 0046 + TC-⑮, implementation LANDED by 06e (`b591f4e`), 27/56 → 56/56. Capital guard magnitude (Part 2 #10): **CP-⑤** rules 가안 2,500/pop — the row's "350 vs 1500" framing was wrong, `capitalGarrison 1500` was never a seal, and the live question was the coefficient's size. CP-① staleness: stamped, and it was **items 1 and 3**, not item 3 alone — R3 retired item 1's designation rule and § 1.8 misfiled it. **06d merged the same day (`34d728d`), so nothing blocks it — this is the frontier and the loop closes here**. **Then the recompute was actually run and found a fourth item underneath the three: which register backs the capital guard** (Part 2 row **16**, 2026-07-31) — CP-① calls the guard register-backed at CP-⑤'s 가안 2,500/pop while MT-② (amended) stores the register at 1,800/pop **per sector**, so seating it the way 06d seats a shield throws at every legal capital. ✅ **RULED 2026-08-01 — capital CP-⑥: realm-wide backing**, the rule ADR 0047 item 5 already gives the opening field army; 0047's header stamped. No value changed. Re-measurement made the choice narrower than the row: over **840** capital candidates the highest locally-backable coefficient is **1,453–1,490** against item 7's floor of **>1,800**, so reading (a) had no overlap at all — the 1.389 ratio omitted the opening field army's ~18% draw, and `r5_s8` has 0 available civilians at any coefficient | **RESOLVED 2026-08-01** — the loop closes. Then the recompute was run **a third time, at claim**, and found a **fourth** item under the three: where a capital sector also carries an ordinary border shield (**179 of the same 840 candidates, 21.3%**), no seal said whether the sector holds shield **plus** guard or guard **instead of** shield. CP-⑥ had measured both readings and deliberately scoped the question out. Ruled the same session — **CP-⑦, additive**. One item is registered rather than absorbed: **row 17**, what a *simultaneous* double capital fall names, whose refusal the user pinned rather than answered |
| 08 | ✅ | ✅ **cleared 2026-08-03, in two steps.** Part 2 #1, #4, #5 and #6 — the whole fog band — closed by the morning's fog grill (`RULINGS.md` ③ / ADR 0048), two of them by ADR 0041 §2 rather than by a ruling. A groundwork pass then found a blocker underneath that no row had: **what a testimony attaches to.** Ruled the same day — the subject is set by whether it can **move** (`RULINGS.md` ④ / ADR 0050), which is neither horn of the recorded either/or. The one derived value it produced (the reporting spread) is sealed at FG-M① and is not a new dial | **RESOLVED 2026-08-05** — implemented `4eff886` + `9a75bf5`, merged, `verify:game` green on all six lanes. Three riders were registered rather than absorbed, all on `docs/SYNC-DEBT.md`: the verification-inputs rule, garrison mobility coupling this ticket's attachment table to the posture-transfer hold, and the knowledge matrix's missing adjacency grade, which ④ decision 3's tracking price now leans on. **Three acceptance items were unsatisfied at close and each moved to its owner** (user decision): the free-contact/purchase inversion is a value question already carried by `docs/SYNC-DEBT.md`, the defensive mirror moved to **ticket 04** because it is UI this ticket built none of, and the testimony history is satisfied as data with its presentation clause left where ③ decision 4 deferred it. Holding the ticket open for those three inverted a dependency — two belong to a ticket that is not downstream of this one — and made `frontier.js` advertise a ticket with no code left to write |
