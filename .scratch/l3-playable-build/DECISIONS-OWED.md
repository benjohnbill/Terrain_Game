...
| 13 | 판세 in-play surface — **new input 2026-08-03**, fog `RULINGS.md` ④ decision 6: an **evidence** surface is not a **verdict** surface, and that distinction may be the whole of the disagreement, since Gate 6 forbids an in-play strategic *verdict* while gate 07's mini-meter carried an evidence read. ④ seals what such a surface must and must not do — aggregate the sector side, refuse to sum the force side visibly, show coverage, assert no position — and deliberately does **not** place it, because placement is ticket 04's to rule | *already registered in* `docs/SYNC-DEBT.md` | | ticket 04 | open |
