...
| 2 | Encirclement threshold — **CLOSED 2026-08-05 by user grill**, sealed at `MAGNITUDE.md` M7 (reading note + § Consequence). **Ruled: 2.2 stands, and this was never two decisions — it is one mis-citation.** The ledger premise was quoting M7, which owns plan thresholds, and its 신중 압박 `~1.1` matches M7 exactly; M7's Encirclement cell carries *two* numbers (threshold 2.2, M4 rout onset R≈1.92) and the premise took the second. Three things made 2.2 forced rather than chosen: M7's derivation is 2.2's **position above** the cliff ("success arithmetically implies rout"), § Toxic band already names that placement as protected, and fog `MAGNITUDE.md` FG-M① *derives* the reconnaissance crossover ρ≈1.49 from 1.92-as-rout-onset — so adopting 1.92 as a threshold would leave one number carrying two meanings in two features. Also recorded, because it is what a re-cut inherits: **2.2's position is derived, its headroom is chosen** — a later pass inherits "above rout onset, with headroom", not the figure. ⚠ **Carries one thing out with it:** the correction moves 1.92 from the needle span's edge to its interior, and nothing had ever placed the cliff on the bar (`eval-r.ts` has no threshold concept). The same grill ruled that **the cliff is read, without a number** — sealed at M7 § Consequence, L1; the treatment is ticket 09's to cut on a live prototype. Hands ticket 11 one item: M7's Flanking row carries an unresolved conditional ("lower to 1.4–1.5 … check at the matchup-fraction stage"), and 11 *is* that stage | `MAGNITUDE.md` M7: **2.2**, described as "the only threshold above rout onset (R≈1.92)" | Ledger: "포위 섬멸 ~**1.92**" — which is the rout-onset figure, not the threshold | tickets 09, 10, 11 | closed 2026-08-05 |
...
| 7 | Plan effect axes — **CLOSED 2026-08-05 by user grill**, sealed at `MAGNITUDE.md` M8 § Consequence. **Ruled: not a conflict — two layers of one method.** The proof is measurable inside M8: `usableValueDamage` has **two `core` holders with different dials** (Raid −15%p→−30%p; Scorched Earth economy base −30%p + population usable −50%p), so `core` cannot be the magnitude. Every M8 number is authored per **(plan, axis)** pair — which *is* ADR 0024's per-axis magnitude, supplied by the numeric pass 0024's own text deferred it to. `core`/`secondary`/`none` is the identity layer for fit ranking and claim blocks (M8 says so in words; `CATALOG.md` calls it "Shape notation" under a user-approved shape-first method). What 0024 rejected was a **plan-level** ranking used *instead of* magnitude, which nothing in the chain does. **0024 is not amended** — header + README carry a clarification stamp only, because its parenthetical read alone condemns the catalog. DOMAIN_MAP's Tier-0 entry carried the same misleading clause and is corrected. ⚠ **One thing was genuinely open and is now sealed: `confidenceGain` stays an axis** (user), with its magnitude owned by fog `MAGNITUDE.md` FG-M① since ADR 0048 — which prices information by *observation type*, not axis, so the axis has no dial of its own and holds the same standing as `controlShift` (headline-owned) and `garrisonDamage` (casualty-curve-owned). **The user holds the option of attaching a plan-axis dial later**; the gap that would justify it is named at M8 (FG-M① has no entry for an information byproduct that produces no observation event — a raid meeting no sortie), and it inherits FG-M①'s own playtest pickup rather than a new trigger | ADR 0024: a **per-axis magnitude**, explicitly "not a primary/secondary classification" | `CATALOG.md`: `core` / `secondary` / `none`, used by all twelve plans | ticket 10 | closed 2026-08-05 |
| 8 | Matchup filled-cell count — **DISSOLVED 2026-08-05 by #9's ruling**, not adjudicated. The two rival tallies disagreed about exactly six cells: the plain-`refuse` cells of the abandonment column, counted as *derived* by the prose/INDEX reading (6 authored / 15) and as *filled* by the table reading (12 / 9). #9 retired that column, so those six cells no longer exist and **both readings now count 5 authored / 9 derived of 14** — measured, not chosen. `MATCHUP.md` and `INDEX.md` restated in the same batch. ⚠ **The definitional question was never answered and could return:** if a future cell is ever authored as a bare `refuse` with no further terms, "is a ✅ mark authorship or status?" is live again. It is ticket 11's to settle then, and nothing today depends on it — the check has no live case, which is why it is recorded here rather than ruled | `MATCHUP.md` prose "15 of 21 cells are empty" and `INDEX.md` "6 authored" | the table itself renders **12** filled cells and 9 empty | ticket 11 | closed 2026-08-05 |
| 9 | The matrix's third defence column — **CLOSED 2026-08-05 by user grill**, sealed at `MATCHUP.md` § The cede rule. **Ruled: the column comes out; abandonment is a rule, not an axis.** Its seven cells were one repeated `refuse` plus one precondition failure, in a table whose only job is to record what the attacker's *method* changes — and the method changed nothing. The decision that made this a design question rather than bookkeeping: a column becomes a third **plan card** beside Stronghold and Delaying, and the declaration's entire value is that it is **free** and leaves the action for elsewhere (ADR 0021's sacrifice loop) — a card hides exactly that. Scorched Earth is not a column either, on independent grounds: its axes are all self-directed and its shape entry says "No battle is sought". Three things already agreed before the ruling — `battle.ts` has two defence methods and no third slot, the slice-2 operational design lists the declaration as a free board verb separate from Scorched Earth, and `MATCHUP.md`'s own column note excludes non-contesting plans. Relocated rather than lost: the isolation precondition ("the abandonment declaration itself is impossible") is now the declaration's availability line in `CATALOG.md`, and the escape-is-a-default counterweight was always borne by the escape-hunting family in the Delaying column. **Dissolves #8** (above) | `MATCHUP.md` column "Strategic Abandonment", against which six cells resolve `refuse` | `CATALOG.md`: "**Abandonment is a declaration, not a plan.** Scorched Earth is the real plan" | ticket 11 | closed 2026-08-05 |
