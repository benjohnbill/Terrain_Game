...
> lives. Last regenerated: 2026-07-11 evening (crisis-ending design
> pass — includes this session's own seals; targeted through this date
> — convenience surface; per the amended law, "may lag canon" covers
> only content older than the current session).
...
## Crisis ending (2026-07-11 evening — RULINGS CE-①…⑫ / ADR 0035; skeleton sealed, body + dials open)

| Term | Gloss | Def |
|---|---|---|
| Uprising fuel (봉기 연료) | per-sector crisis pressure = f(cumulative scar, mobilization intensity); land-bound, inherited on conquest | MA CE-③ |
| Scar ledger (흉터 장부 · 원한) | permanent violence record per sector (raid/occupation events); NEVER decays — the land remembers; fog-spec visibility (L3) | MA CE-③ |
| Standing rebels (잔존 반란군) | map-visible per-sector rebel stack: grows by rate(t)×scar, shrinks under suppression, secedes past threshold, stands as denial mass | MA CE-⑧ |
...
| Suppression (진압) | crisis payment, SHIELD-natured (garrisons/reserves/register; field optional); suppressing itself scars a little (dial, 0 allowed) | MA CE-⑧/⑩ |
| Secession by neglect (방치 이탈) | N unpaid turns (가안 2) → the sector defects to the rebels; a consequence, never an action | MA CE-⑥ |
| Pacification condition (평정 조건) | standing rebel mass counts in the unassailable denial side — crown = conquest + pacification | MA CE-⑦ |
| Total-war overlay (총력전 오버레이) | calendar-staged death of the promise enforcer (truces/settlements void); same war machine, no new arithmetic | MA CE-⑨ |
