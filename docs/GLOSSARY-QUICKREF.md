...
> birthplace SYNCED: aging constitution P1-P3, 징집 명부 re-founding,
> Surge Draft Model, start-state coordinates now live in match-arc
> GLOSSARY/RULINGS MT-①…④ + MAGNITUDE M13/M13a + DOMAIN_MAP — the
> C-loop rows below are pointers to those homes, no longer staging).
