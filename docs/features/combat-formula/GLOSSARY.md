...
| Term (한국어) | Definition | Summary | Value / range | Source | Status |
|---|---|---|---|---|---|
...
| Standing rules (상비 규칙) | World rules that tick each turn reading persistent state: **starvation** — a supply-ledger pump whose sole output is convex substance loss, with no stages and no capability flips — and **recovery**, gated by supply and by ground (scorched ground denies it). *(Re-cut 2026-07-28: the earlier staged severity holding→attack-incapable→defenseless, including stage 2 as an availability gate, is SUPERSEDED by war-model slice-2 §2; garrison regeneration left this list when replenishment became paid recruitment or physical transfer rather than a standing pulse, ADR 0045.)* |  | rates → stage 5; constraint: starvation must outpace an unsupplied 2–3-sector advance | ADR 0026, M4; supersession: war-model slice-2 §2, ADR 0045 | AGREED (rates 가안 → stage 5; starvation re-cut 2026-07-28 · L0) |
