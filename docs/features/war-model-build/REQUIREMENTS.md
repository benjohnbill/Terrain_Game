...
| A2 | **Combat resolves atomically in its turn** — no operation-in-progress spans turns; long campaigns are emergent chains | ADR 0026 | ~~multi-turn siege→field→cascade→capital conveyor accumulating `war.stalled`~~ — **`war.stalled` DELETED 2026-07-16 (ticket 11)**; the L2 stage conveyor itself is retired-not-rebuilt (ADR 0037) and the slice-2 loop resolves atomically | **CLOSED 2026-07-16** — the 2-turn stall exit that fired before the decisive battle no longer exists (deleted, not disabled: no stall/patience/near-miss counter survives, pinned by `tests/crisis-arc.test.js`). Replacement = read-driven settlement (`js/bot-exit.js`), which carries no timer by construction | **P1 — slice 1 SEALED 2026-07-13** (atomic `resolveEngagement` chain, WM-①); stall exit retired ticket 11. **Measured consequence, not hidden:** removing the timer did not make fizzling wars decisive — it made them run to the envelope (L2 post-retirement: 78.8% `stallPeace` → 72.1% never-ending; the slice-2 loop's own figure is metric 5's `unresolved` row) |
...
|---|---|---|---|---|---|
| C1 | **War appetite = the opportunism read** — check the ratio against screen + garrison when a rival's field army is pinned on another front (the motion sheet-8 had to hand-script). *SPEC_GAP ① closed at design level — slice-2 spec §7 (window arithmetic, per-detachment reach cones).* | match-arc SPEC_GAP ①; slice-2 spec §7 (design SEALED 2026-07-14, WM-②) | static ~1.7 declare gate → symmetric board freezes | **YES** | **P1 — design SEALED, implementation next** |
| C2 | **Stall→white-peace auto-exit is BOT POLICY ONLY** — an L2 approximation of "when a rational court takes the 0% rung"; **never force-closes a war over a human player** | match-arc CE-⑲; slice-2 spec §9 (redesign SEALED 2026-07-14, WM-② — read-driven settlement, ADR 0038 will path) | patience 2 / near-miss R<1.1 → directly generates the 77% white-peace | **YES** (bot artifact) | redesign SEALED — implementation next |
| C3 | Bot info = contact-gating (only wars factions it has met); walls are public info | fog-of-war-discovery | implemented | no | reuse |
| C4 | ~~Single~~ mobile field army's allocation across simultaneous wars — **free division sealed** (slice-2 spec §4; 20% screen harness rule retired; SPEC_GAP ② design opened) | match-arc SPEC_GAP ②; slice-2 spec §4 (2026-07-14, WM-②) | 20% screen harness rule (retired with L2) | maybe | design opened — allocation policy rides the window read |
...
| D3 | **Reactive reserve (M9)** — route-connected same-province stock awakens, arrives march-worn ×0.5, stays garrisoned; reserve mass = field-army counter + M9 fill (field-army layer now per-detachment — FG-⑥/⑩ stamps, slice-2 spec §4) | combat MAGNITUDE M9; force-geography FG-⑤/⑩ (amended 2026-07-14) | L2 harness (`FG_BOARD_GAAN`) | maybe | P2 (engine adoption, slice 4) |
| D4 | **Supply starvation** as a standing world rule — realized as the fatigue supply ledger: recovery lock + starvation state converting continuously (convex) into substance loss; substance-only (staged capability severity superseded — slice-2 late ruling); ends when route restored by an action | ADR 0026 (amended 2026-07-14); slice-2 spec §2 | structure present; rates 가안 | maybe | **P1-adjacent — designed in slice 2, implemented with the fatigue system** |
