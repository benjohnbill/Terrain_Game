...
| Term (한국어) | Definition | Summary | Status |
|---|---|---|---|
...
| 야전군 (field army) | A realm's mobile main force — the user's device for controlling R: geometry-bound (position, movement, fatigue), **freely divisible and mergeable** as turn actions (conservation of tiredness: fatigue × men invariant across split/merge), land-derived; losses are permanent currency. Repositions reactively (FG-⑥ deficit×value, now per-detachment); the operational half of the two-layer reserve (FG-⑩, beside M9 tactical fill). In the 결전 it fights open-field by mode at its fatigue-curve effectiveness. Its reach/pinning (per-detachment reach cones) is the branch switch between 결전 and a front's FALL — the board the opportunism read plays. *(Superseded clauses, 2026-07-14 WM-② / slice-2 spec §4: "one at a time" → free division; "arrives march-worn, FG-⑩ rider ii" → the rider dial is retired, subsumed by the fatigue conversion curve, spec §2.)* |  | **AGREED** (2026-07-13, WM-①; amended 2026-07-14, WM-②) |
...
| 모병 (recruitment) | The single MVP economy→mass conversion: a primary action moving bodies from the 징집 명부 into serving status toward the force limit (구칭 national cap), paid from treasury yield. Each request names a controlled front sector, commit, field/garrison posture, and permitted field destination/affiliation; field and garrison recruits are next-turn-ready, not current-turn combat substance. **Priced by the 서지 모병 model (MT-③), not a flat +10%**. The temporary-levy track (공세 동원, ADR 0009 role 3) is a **reserved seat**. Force-adjustment stack: ① recruitment (create, player) / ② garrison replenishment (paid recruitment or physical transfer, never a free pulse — ADR 0045) / ③ standing-force stationing (deploy, player) / ④ commit lever + reserve (activate, per-turn). History: RULINGS MT-①/③/⑥. |  | **AGREED** (2026-07-04; amended 2026-07-26 MT-⑥) |
| 징집 명부 (conscription register · 구칭 인력 풀 / manpower pool) | "군사력이 될 수 있는 모든 신체 인구" — the total living draftable bodies a realm holds, **land-derived and finite within a match**, persistently accounted **by sector**. Every serving body retains **sector origin**; recruitment moves civilian→serving without changing that origin register, while permanent loss reduces the same origin share. Land transfer moves the remaining civilian register; already-serving origin composition stays with its force. capPerPop 600 is the derived sibling constant — the **sustain fraction ⅓**. Supersedes "1.5 × initial military" (MT-②). UI owes: register remaining + 동원 강도 meter. *(This row said "by province" and "province origin" until 2026-08-03. **ADR 0047 moved origin accounting from province grain to sector grain on 2026-07-31** and stamped MT-⑥ with that delta; the definition row itself was not carried over, so the birthplace tier read one grain while its own ruling tier read the other. Found while writing fog `RULINGS.md` ④, whose immobile half rests on this exact property — a serving body keeping its **sector** origin wherever it stands is what stops a sector's mobilization reading from marching away.)* History: RULINGS MT-②/⑥; ADR 0047. |  | **AGREED** (2026-07-04; amended 2026-07-26 MT-⑥; grain corrected 2026-08-03 per ADR 0047) |
...
| Limbo (점령 무귀속) | Occupied-but-untransferred land: sectors count toward NEITHER side's derived quantities — the defender lost them (occupied land pays no taxes), the attacker has not integrated them. **An interval, not an end state (ADR 0044):** conquest integrates land on the ADR 0022/0029 ripening lag, so limbo is the phase before integration. Historically also resolved at settlement or stall/white-peace (id-exact return), both retired for the duel by ADR 0042. History: `RULINGS.md` OG-③. |  | **AGREED** (2026-07-10, OG-③; amended 2026-07-26, ADR 0044) |
