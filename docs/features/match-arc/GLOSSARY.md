...
| 서지 모병 (Surge Draft Model) | Dynamic pricing of the recruit primary on two axes. **Depth**: unit price = a continuous piecewise-linear marginal curve over 동원 강도; a realm's simultaneous sited requests receive one bill over fulfilled aggregate mass (integral pricing — no cliffs or split-order advantage). **Size**: commit points purchase the sealed +1%p conversion; local civilian and posture headroom shortages prorate deterministically before treasury affordability. Garrison replenishment pays the same curve. Knees, multipliers, surge rate → magnitude session. History: RULINGS MT-③/⑥; UI: DISPLAY-DEBT. | **AGREED-structure 2026-07-07; amended 2026-07-26 MT-⑥** |
