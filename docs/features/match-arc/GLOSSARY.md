...
| Sudden-death crisis ending (위기 종결) [조어] | The match-end mechanism: an internal, karma-driven uprising arc (turn 25 onset → turn 35 hard end) that erodes the deadlock — shield order and coalition glue — and forces the world toward the hegemony decision point, which remains the SOLE winner rule. No dice; hermit-kingdom prep is real (governance itself); the turn-35 residual is a Westphalian draw, never a judged verdict. Skeleton sealed by the crisis-ending grill; rebellion-body mechanics still open (rider). History: `RULINGS.md` AB-③ → CE-①…⑫; ADR 0034 → 0035. | **AGREED** (skeleton SEALED 2026-07-11, CE-series · L0; rebellion-body pass pending) |
| Uprising fuel (봉기 연료) [조어] | Per-sector crisis pressure base: f(cumulative scar ledger, mobilization intensity). Land-bound — fuel belongs to the sector, not the owner — and inherited on conquest. Read by rolling assessment each crisis turn. L2 approximation: realm mobilization intensity spread uniformly over held sectors. History: `RULINGS.md` CE-③. | **AGREED** (2026-07-11, CE-③ · L0) |
| Scar ledger (흉터 장부 · alias 원한) [조어] | The permanent per-sector violence record: accrues on raid and armed occupation (per-event increments = dials; peaceful-cession increment = open question), **never decays** — "the land remembers." Distinct from usable (economy, which heals). Visibility follows the fog spec: own 100% / scouting confidence / former owner keeps 100% only in what it witnessed (L3 layer; L2 reads true values). History: `RULINGS.md` CE-③. | **AGREED** (2026-07-11, CE-③ · L0; increments 가안) |
| Standing rebels (잔존 반란군) [조어] | The crop of the soil-and-crop model: a map-visible per-sector rebel stack that grows each crisis turn by rate(t) × scar, shrinks under suppression, defects the sector past the secession threshold, and stands in the decision point's denial term. Never a marching AI army. History: `RULINGS.md` CE-⑧. | **AGREED** (2026-07-11, CE-⑧ · L0) |
...
| Suppression (진압) | The crisis-turn payment: bodies committed against standing rebels, **shield-natured** — paid from garrisons, reserves, and register draws; field-army diversion optional, never forced. Suppressing adds a small scar increment ("the sword scars wherever it swings" — magnitude dial, 0 allowed). Resolution mechanics = next-grill branch. History: `RULINGS.md` CE-⑧/⑩. | **AGREED** (2026-07-11, CE-⑩ · L0; resolution open) |
| Secession by neglect (방치 이탈) [조어] | A sector whose suppression bill goes unpaid N consecutive turns (가안 N=2) defects to the rebels — abandonment is a consequence of sustained refusal, never an explicit action. History: `RULINGS.md` CE-⑥. | **AGREED** (2026-07-11, CE-⑥ · L0; N 가안) |
| Pacification condition (평정 조건) [조어] | Standing rebel mass (conversion coefficient = dial) counts into the unassailable check's denial side — own and others' alike. The crown requires conquest + pacification; nobody is unassailable over a continent in flames. Self-prices burn-and-rush. History: `RULINGS.md` CE-⑦. | **AGREED** (2026-07-11, CE-⑦ · L0; coefficient 가안) |
| Total-war overlay (총력전 오버레이) [조어] | The arc's restraint decay: peacetime guarantees die in calendar stages (truce shortening → void; settlement narrowing → void; stage table = dials) because no authority can enforce promises any longer — same war machine, no new arithmetic, no separate plan catalog. History: `RULINGS.md` CE-⑨. | **AGREED** (2026-07-11, CE-⑨ · L0; stage table 가안) |
