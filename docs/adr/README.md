...
| 0041 | Environment Isolation — Marketing Landing vs Game Runtime, and the Reference Archive | 2026-07-17 | Accepted (amended by 0050) | → Amends 0016, 0028 (Stage 2 native shell = declared destination; Firebase = landing only; `js/`/`tests/`/L2 = reference archive, not build source; L3 source occupies its own space) · ← **Amended by 0050 (2026-08-03)**: the landing may carry a playable demo of the L3 build, bounded to copying the emitted bundle as an opaque artifact; "landing surface **only**" corrected; distribution target and the protected direction of isolation unchanged |
